...
import { http, BULL_BOARD_BASE } from '../api/client';
...
export default function SettingsPage() {
  ...
  return (
    <div>
      <PageHeader title="Configurações" subtitle="Sincronização, saúde e integrações" />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="card p-5">
          ...
          <p className="text-sm text-slate-400 mb-3">
            Descobre filas (<code>bull:*:id</code>) nos Redis habilitados e registra as novas.
          </p>
          ...
          {sync.isSuccess && (
            <div className="mt-4 space-y-1 text-sm">
              <p className="text-slate-300">
                {sync.data.totalDiscovered} filas encontradas ·{' '}
                {sync.data.totalCreated} novas
              </p>
        ...
        <div className="card p-5 md:col-span-2">
          ...
          <p className="text-sm text-slate-400">
            ...
            <code className="text-brand-hover">{BULL_BOARD_BASE}</code> e embutido
            dentro de cada fila. Para abrir uma visão consolidada de todas as filas
            habilitadas, acesse{' '}
            <a
